...
static bool TestBpeEncodeDecodeAndSpecialToken() {
  ...
  // Rank-min merge: h e -> he l -> hel l -> hell o -> hello (id 7).
  ...
  // BOS is prompt-layer work; Encode must not prepend it.
  MINI_LLAMA_ASSERT_TRUE(hello[0] != tok->bos_id());
  ...
  // Special tokens are intercepted before the BPE merge loop.
  std::vector<int> mixed = tok->Encode("<|im_start|>hello");
  MINI_LLAMA_ASSERT_EQ(mixed.size(), 2u);
  MINI_LLAMA_ASSERT_EQ(mixed[0], 8);
  MINI_LLAMA_ASSERT_EQ(mixed[1], 7);

  // Unknown pieces fall back to unk_id after byte-to-unicode mapping.
  std::vector<int> unknown = tok->Encode("z");
  MINI_LLAMA_ASSERT_EQ(unknown.size(), 1u);
  MINI_LLAMA_ASSERT_EQ(unknown[0], tok->unk_id());
