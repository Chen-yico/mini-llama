...
static bool TestGgufAlignmentEmbeddedNullRejected() {
  ...
  AppendString(data, "general.alignment");
  AppendU32Le(data, kGgufTypeString);
  std::string dirty("64");
  dirty.push_back('\0');
  dirty.append("abc");
  AppendString(data, dirty);

  std::filesystem::path path = TestTempPath("alignment_embedded_null.gguf");
  WriteBinaryFile(path, data);
...
static bool TestGgufDuplicateTensorNameRejected() {
  std::vector<char> data;
  data.insert(data.end(), {'G', 'G', 'U', 'F'});
  AppendU32Le(data, 3);
  AppendU64Le(data, 2);
  AppendU64Le(data, 0);
  AppendTensorInfo(data, "weight", 1, 0);
  AppendTensorInfo(data, "weight", 1, 4);

  std::filesystem::path path = TestTempPath("duplicate_tensor.gguf");
  WriteBinaryFile(path, data);

  GgufReader reader;
  const bool ok = reader.Load(path.string());
  std::filesystem::remove(path);

  MINI_LLAMA_ASSERT_TRUE(!ok);
  MINI_LLAMA_ASSERT_TRUE(reader.load_error == "duplicate tensor name: weight");
  return true;
}

static bool TestGgufOversizedStringRejected() {
  std::vector<char> data;
  data.insert(data.end(), {'G', 'G', 'U', 'F'});
  AppendU32Le(data, 3);
  AppendU64Le(data, 0);
  AppendU64Le(data, 1);
  AppendU64Le(data, 16ULL * 1024 * 1024 + 1);

  std::filesystem::path path = TestTempPath("oversized_string.gguf");
  WriteBinaryFile(path, data);
...
static bool TestGgufArrayMetadataIsSummarized() {
  std::vector<char> data;
  data.insert(data.end(), {'G', 'G', 'U', 'F'});
  AppendU32Le(data, 3);
  AppendU64Le(data, 0);
  ...
  AppendU32Le(data, kGgufTypeString);
  AppendU64Le(data, 2);
  AppendString(data, "hello");
  AppendString(data, "world");

  std::filesystem::path path = TestTempPath("array_metadata.gguf");
  WriteBinaryFile(path, data);
  ...
  MINI_LLAMA_ASSERT_TRUE(ok);
  MINI_LLAMA_ASSERT_EQ(reader.metadata.size(), 1);
  MINI_LLAMA_ASSERT_TRUE(reader.metadata[0].value_str == "[2 x string]");
  return true;
}
...
static struct GgufTestRegistrar {
  GgufTestRegistrar() {
    ...
    RegisterTest("gguf_alignment_embedded_null_rejected",
                 TestGgufAlignmentEmbeddedNullRejected);
    ...
    RegisterTest("gguf_duplicate_tensor_name_rejected",
                 TestGgufDuplicateTensorNameRejected);
    RegisterTest("gguf_oversized_string_rejected",
                 TestGgufOversizedStringRejected);
    RegisterTest("gguf_array_metadata_is_summarized",
                 TestGgufArrayMetadataIsSummarized);
