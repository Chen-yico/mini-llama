...
static bool TestCommonPrefixLengthAndContextTruncate() {
  MINI_LLAMA_ASSERT_EQ(CommonPrefixLength({1, 2, 3}, {1, 2, 9}), 2);
  MINI_LLAMA_ASSERT_EQ(CommonPrefixLength({1, 2}, {1, 2, 3}), 2);
  MINI_LLAMA_ASSERT_EQ(CommonPrefixLength({}, {1}), 0);
  ...
  MiniLlamaContext ctx(&model);
  ctx.token_history = {1, 2, 3, 4};
  ctx.pos = 3;

  const std::vector<int> tokens = {1, 2, 9};
  const size_t cached_prefix_len = 2;
  const size_t context_prefix_len = CommonPrefixLength(ctx.token_history, tokens);
  const size_t prefix_len = std::min(cached_prefix_len, context_prefix_len);
  MINI_LLAMA_ASSERT_EQ(prefix_len, 2);

  ctx.token_history.resize(prefix_len);
  ctx.pos = static_cast<int>(prefix_len - 1);
  MINI_LLAMA_ASSERT_EQ(ctx.token_history.size(), 2);
  MINI_LLAMA_ASSERT_EQ(ctx.pos, 1);
  return true;
}
...
static struct KvCacheTestRegistrar {
  KvCacheTestRegistrar() {
    ...
    RegisterTest("context_common_prefix_and_truncate",
                 TestCommonPrefixLengthAndContextTruncate);
  }
