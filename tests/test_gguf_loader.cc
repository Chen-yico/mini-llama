...
static bool TestGgufLoaderLoadsTinyFixture() {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    MINI_LLAMA_ASSERT_TRUE(lw.wq.type == QuantType::kF32);
  ...
  MINI_LLAMA_ASSERT_TRUE(model.lm_head.type == QuantType::kF32);
...
static bool TestGgufLoaderLoadsRealModelWhenAvailable() {
  ...
  MINI_LLAMA_ASSERT_TRUE(model.config.rope_type == RopeType::kNeoX);
  MINI_LLAMA_ASSERT_TRUE(model.layers[0].wq.type == QuantType::kQ80);
  MINI_LLAMA_ASSERT_TRUE(model.layers[0].wq.f32_data.empty());
  MINI_LLAMA_ASSERT_TRUE(!model.layers[0].wq.q8_0_data.empty());
  MINI_LLAMA_ASSERT_TRUE(model.lm_head.type == QuantType::kQ80 ||
                         model.lm_head.type == QuantType::kF32);
  return true;
}
